...
Void or Delete the Check That Applied the Debit:

In CHECKS.E2, void or delete the check that applied the debit to the invoice (e.g., check A901).
Void the Target Check:

After voiding or deleting the first check, proceed to void the check that could not previously be voided (e.g., check 140404).
Verification
Confirm that both checks have been successfully voided or deleted in CHECKS.E2.
Ensure that the offset between the prepayment and vendor invoice has been properly reversed.
Verify that the vendor’s account reflects the correct status after the voids.
