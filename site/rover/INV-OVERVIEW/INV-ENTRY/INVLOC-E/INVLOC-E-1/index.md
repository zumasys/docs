...
**Cost Group** Enter the cost group which will be validated against [ INV.CONTROL ](../../INV-CONTROL/index.md) . The cost group is used to group inventory locations together with a common costing method as defined on [ INV.CONTROL ](../../INV-CONTROL/index.md) . The cost group should not be changed on existing records if there are existing quantities on hand in this location.   
