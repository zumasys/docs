...
- [Data Entry Procedures](INV-ENTRY/index.md)
- [Reports and Inquiries](INV-REPORT/index.md)
- [Update Processes](INV-PROCESS/index.md)
...
**Setup Procedures**  
There are two setup procedures which need to be addressed before beginning with the normal flow of activities. The first is defining the valid inventory locations. These must be present before inventory transactions may be entered. The procedure used to define these locations is [INVLOC.E](INV-ENTRY/INVLOC-E/index.md) . The second is the setup of default and control information in the [INV.CONTROL](INV-ENTRY/INV-CONTROL/index.md) procedure.

**Flow of Activities**  
The only ongoing activity associated with the Inventory module is the entry of inventory transactions. There are two procedures available to enter these transactions, they are [IT.E](INV-ENTRY/IT-E/index.md) and [IT.E2](INV-ENTRY/IT-E2/index.md) . The result of using either one of these procedures is the same, the difference is that [IT.E](INV-ENTRY/IT-E/index.md) is used to enter one transaction at a time while [IT.E2](INV-ENTRY/IT-E2/index.md) is used to enter several transactions at once. [IT.E](INV-ENTRY/IT-E/index.md) should be used in situations where you want to enter a single transaction from time-to-time and want it to update the inventory balance immediately. An example of this would be the stock clerk recording the transaction every time they issue a part to someone. [IT.E2](INV-ENTRY/IT-E2/index.md) should be used when several transactions have been accumulated and are entered all at once. A situation where you would choose to use this procedure is when the stockroom records each issue of material on a sign-out sheet which is later forwarded to data entry. The inventory balance is then updated when the data entry clerk enters the transactions from the list.
...
The Usage file is updated by the procedure [USAGE.P1](INV-PROCESS/USAGE-P1/index.md) . This procedure determines the total usage for each part for the month specified by accumulating the appropriate inventory transactions for the period. The procedure should be run at the end of each month.
