...
**Form Details**  
[ Form Details ](AP-P5-1/README.md)   

**Purpose**  
The AP.P5 procedure is used to purge inactive AP records from the data base.
The criteria used to deterime which records are purged is based on a user
defined cut-off date. All accounts payable records which have a paid date that
is less than or equal to the cut-off date, and whose checks are already
deleted or have a clear date after the cut-off date will be deleted. Any
associated checks records which are still on file will also be deleted.
