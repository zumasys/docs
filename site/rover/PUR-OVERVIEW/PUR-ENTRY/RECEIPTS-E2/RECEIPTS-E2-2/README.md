...
for a phantom part with quantity received. This data is loaded from the
original receipt.  
...
**Comp Lot** Contains the lot number assigned to the receipt line item. It may
not be changed.  
...
**Parent WO** Contains the work order number the part will be removed from if
the location is a work in process location. This is derived from the parent
