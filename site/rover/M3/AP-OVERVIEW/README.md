...
AP.E are either automatically assigned numbers by the system or
...
is the interface file to the general ledger module. In the AP.E
procedure, the register date entered is assigned to these records. In the
CHECKS.E procedure, the check date is assigned. This gives the
...
which reference the purchase order. It is used by the AP.E
...
payable control record AP.CONTROL must also be setup
...
directly through AP.E  for non-receipt items. Third, items may be
created automatically by the APR.P1  batch recurring entry
...
actually posted through the CHECKS.P1  process. The check
...
AP.P5  and CHECKS.P2  procedures.
