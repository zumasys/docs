...
**Record Id**|  Enter the record ID for which you wish to list all AR Register
records. Examples of record ids would be the cash id that is assigned by the
system in [cash.e](../Cash-e/README.md), a/r invoice number, shipment number, rma
receipt number, etc. Please remember that when a shipment or rma receipt is
posted, it is the shipment or rma receipt number that is posted to the ARREG
records. If any changes are made to the a/r item in [AR.E](../AR-E/README.md), it will
be the ar.id that is posted to the arreg records. Therefore, if you wish to
view all transanctions for a given item, you may need to run this report for
multiple record ids.
