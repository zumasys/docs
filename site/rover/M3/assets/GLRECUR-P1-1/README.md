...
**Fiscal Date**|  Enter the date which is to be used to determine which
recurring items are to be processed. If the recurring item is active (status =
'A') and the date entered here falls between the start and end fiscal dates on
the GLRECUR record, then the records will be processed.

**Glrecur Id**|  If you only wish to post certain recurring entries, enter the
recurring ids here.
