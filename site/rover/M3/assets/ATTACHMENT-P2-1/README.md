...
**Source Folder**|  Select the folder that contains the attachments to be
moved.

**Destination Folder**|  Select the folder into which the attachments are to
be moved.

**M3 File Name**|  Typically, each attachment is associated with an M3 data
file. This field allows you to enter an M3 file name that will restrict the
attachments that are moved to only those associated with the file name.

**Start Date**|  You may restrict the items that are moved by specifying a
date range. If you specify a date in this field then only the attachments with
a date greater than or equal to that date will be moved.

**End Date**|  You may restrict the items that are moved by specifying a date
range. If you specify a date in this field then only the attachments with a
date less than or equal to that date will be moved.

**Maximum Items**|  If you only want to move a portion of the attachments you
may indicate the maximum number to be moved in this field. If left blank then
