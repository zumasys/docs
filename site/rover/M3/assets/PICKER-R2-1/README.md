...
**Work Order Numbers**|  Enter the un-pulled work order that you want to
display or print. If the work order is not being backflushed, the status of
the work order must be N, O or R. If the work order is scheduled for
backflushing the status on the work order must be P and no material can have
been issued to the work order.

**Shortages Only**|  Check this box if you only want to include items with a
shortage on the listing.

**I)nv Balance or M)RP Proj**|  There are two options available to determine
what items on the pick list will be short. If you enter the letter "I" the
system will determine what the shortages would be if you pulled the kit
immediately from the inventory location specified on the work order. If the
letter "M" is entered then the system will check the MRP projected balance for
each part on the work order release date. Since the MRP projected balance
represents all of the known demand for the part, it will give you a worst case
shortage situation.
