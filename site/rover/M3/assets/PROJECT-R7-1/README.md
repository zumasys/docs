...
**Projected Date Flag**|  Check the box to use projected dates for determining
if a task is past due. If unchecked, the planned completion dates will be used
for comparison. If box is checked, projected completion dates will be used
only if they have been entered for the task, otherwise planned dates will be
used. The comparison is made against the cutoff date entered.

**Cut-off Date**|  Enter a cutoff date to be used to compare to the task's
projected or planned complete date to determine if a task is past due for
completion. If no cutoff date is entered, the current date will be used.
