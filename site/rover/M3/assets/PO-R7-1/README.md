...
**Vendor Numbers**|  Enter the numbers of all of the vendors you want to
included in the listing or leave this field blank to include all vendors by
vendor name. If you do not know the vendor number, there is an option in the
help menu for this prompt which allows you to select the vendor by name.

**Cut-off Date**|  You may limit the listing to include only those purchase
orders that have been closed after the date specified. If left blank then all
orders are included.

**Exclude Totals**|  Check this box if you wish to exclude totals from the
report. This option is usually exported when exporting the data to Excel or
the grid viewer.
