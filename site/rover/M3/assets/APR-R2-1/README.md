...
**Expiration Date**|  Enter the expiration date to be used in this report. The
expiration date is the last date that a payment is scheduled for on a
recurring item. Recurring items that have been flagged as perpetual items will
have not expiration date.

**Sort By**|  Select the method you wish to sort the report by. If no entry is
made in this field, the report will be sorted by the APR.ID.
