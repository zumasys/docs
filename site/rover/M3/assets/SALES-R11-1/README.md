...
**By R)evenue or M)argin**|  Enter the letter "R" if you want the report
sorted based on the sales revenue, or enter "M" to sort based on the margin
(revenue - cost).

**A)scending D)escending**|  Enter the letter "A" if you want the report
sorted in ascending sequence, or "D" for descending sequence. If no option is
specified the report will be done in ascending sequence.
