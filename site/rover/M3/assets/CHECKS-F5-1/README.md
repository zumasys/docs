...
This field is entered only if you are going to print checks via the
...
to be printed in the "Check Number(s)" field.

**Starting Check**|  Enter the number of the first check to be printed. If a
check is wasted due needed to be fed through the printer, it must be voided
with the [CHECKS.E2](../CHECKS-E2/README.md) procedure. Any checks which are used for
the form alignment process are voided automatically.
