...
end. All transactions after this date will be ommitted from the listing.

**Min Unit Price**|  Enter the minimum unit price of the shipment line item.
If the ship item unit price is greater than or equal to the minimum it will
qualify for the report.

**Max Unit Price**|  Enter the maximum unit price of the shipment line item.
If the ship item unit price is less than or equal to the maximum it will
qualify for the report.

**Exclude Non-Inventory Items**|  Check this box if you want to exclude
shipments of non-inventory items (no part number) from the report.

**Scheduled or Cust Reqd Date**|  Enter an "S" to base delivery performance on
the SO scheduled date. Enter an "R" to base it on the SO customer required
date. The default is "S". If "R" is chosen, and no customer reqd date is on
the order, then schedule date is used.

**Customer**|  If you wish to run this report for selected customers, enter
the customer numbers here.
