...
**Approved**|  Enter "Y" to show all unposted ECN's already approved. Enter
"N" to show unposted ECN's needing approval. Enter "B" or leave blank to show
both.

**Approval Type**|  Enter an approval type if you only want to select ECN's
with a specific approval type that needs approval, or that has been approved.

**Sort By**|  Enter the method you wish to sort the report by. If no entry is
made in this field, the report will be sorted by ECN ID.
