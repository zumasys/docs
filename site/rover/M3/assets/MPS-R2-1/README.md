...
**C$X42**|  Enter the cost group which will be validated against
[INV.CONTROL](../INV-CONTROL/README.md). The cost group is used to define the costing
method and will be used to determine which unit cost to use for the report.

**A)verage / S)tandard Cost**|  Enter the letter "A" if you want to use the
current average cost for each part, or enter "S" to use standard cost. If no
entry is made then standard cost will be used. The system will preload the
cost method defined in the [INV.CONTROL](../INV-CONTROL/README.md) procedure for the
cost group entered.
