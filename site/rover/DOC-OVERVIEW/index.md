...
- [Data Entry Procedures](DOC-ENTRY/index.md)
- [Reports and Inquiries](DOC-REPORT/index.md)
...
**Prerequisites**  
Entry of the control information in [Doc.Control](DOC-ENTRY/DOC-CONTROL/index.md) is required before any documents can be added to the system. This includes userids and roles for those users that will be involved as administrators, editors, or signers of documents. Additionaly, the server file paths where the document files will reside need to be defined.
...
**User Interface**  
The user interface to the document module consists of one main data entry screen, [DOC.E](DOC-ENTRY/DOC-E/index.md) , which administrators use to add or change general document data, and to start a document. Additionally, the [DOC.E2](DOC-ENTRY/DOC-E2/index.md) screen displays all the same detail data as [DOC.E](DOC-ENTRY/DOC-E/index.md) but is not updatable. This screen is used by authorized editors and/or signers of documents to view more detailed information than is visible in the inquiry screen, [DOC.Q](DOC-REPORT/DOC-Q/index.md) . Both [DOC.E2](DOC-ENTRY/DOC-E2/index.md) and [DOC.Q](DOC-REPORT/DOC-Q/index.md) allow editors to check in/out a document and signers to sign off on a document.
  
In addition to these procedures there is a set of notifications that are optional based on a setting in [Doc.Control](DOC-ENTRY/DOC-CONTROL/index.md) . These notifications happen as follows:
...
The notification is sent as an alert viewable from Message.Q and the message has a link to [DOC.E2](DOC-ENTRY/DOC-E2/index.md). The message will be emailed to the user if the user's particular security settings have that option specified.

**Setup Procedures**  
[Doc.Control](DOC-ENTRY/DOC-CONTROL/index.md) must be set up before documents can be created in [DOC.E](DOC-ENTRY/DOC-E/index.md).
  
The following user roles are defined in [DOC.CONTROL](DOC-ENTRY/DOC-CONTROL/index.md):
...
If documents will need signoffs (approvals), then signoff types need to be defined in [Doc.Control](DOC-ENTRY/DOC-CONTROL/index.md) . Users that will be signers will need to be assigned a type. Types are user defined such as departments, sections, or levels of management.

**Flow of Activities**  
The flow of activites in the document module begins wth starting a document in [DOC.E](DOC-ENTRY/DOC-E/index.md). This must be done by an administrator. The status of a new document is "Draft". A revision must also be entered. The initial document must be retrieved using the "Start Doc" button. Once that is done, it is allowed to be edited. Editors and signers are added specifically to a document.
...
Once all approvals are complete, if notifications are turned on, the administrator will be notified, and can change the document status to "Released". The release process "publishes" the current revision. It is now viewable to general users that have view access to [DOC.Q](DOC-REPORT/DOC-Q/index.md).
  
For existing documents that have been released, and are needing a revision, the adminstrator should enter a pending revision and change the status to "Change in Process". Once this is done, the flow is the same as described previously for the new document. Once this revision is released, then the current version of the document becomes the prior version, and the new revision becomes the current published version. All prior versions are viewable in [DOC.E](DOC-ENTRY/DOC-E/index.md) or [DOC.E2](DOC-ENTRY/DOC-E2/index.md).
  
[DOC.Q](DOC-REPORT/DOC-Q/index.md) can be used at any time to search for documents in various statuses, or to search by keywords or categories. In addition, there are various lookups activated by double clicking the Document ID.  
  
The last review date can be updated at any time, and there is a report, [DOC.R1](DOC-REPORT/DOC-R1/index.md) that can be used to list documents needing review based on a user entered time parameter.
