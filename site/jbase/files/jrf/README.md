...
| **-P** | Some files are heavily used, create lots of overflow, but are empty. Sizing will be based on maximum allocated overflwow. |
...
- If the `-HJBC` option is specified, it supercedes all other options and only one file can be converted at a time.
- Options  `-HJBC`, `-Hmongo`, `-K` and `-X` are only available on jBASE 5.8 forward.

## Warning

>The **jrf** utility should ONLY be executed when users are not connected to the database otherwise data loss can occur. In other words, the file should not be opened by any other process.
>  
