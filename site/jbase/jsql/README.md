...
```
>Note in the above example no PRIMARY KEY was specified.
```
...
LASTNAME          A            3            LASTNAME                               L               30
 ...
 4 Records Listed
```
As you can see the dictionary portion of the file generated by the CREATE TABLE command was populated including a default **RECID** column
which was added and will auto increment starting at **1**. The next key value is stored in the DESCRIPTION field in the [Extended Dictionary](../files/extended-dictionary/README.md) of **RECID**.
...
List 'PICK' exited
```
One of the advantages of using **SQLSELECTL** instead of regular **{S}SELECT** is that in situations where you have a large table (file)
and you want to get a result set based on a value in another table (file) that is (ideally) indexed.

Let's propose that CATEGORY instead of being a VARCHAR was an INT and keyed to another table (file) called CATEGORY.
```
jsh -->SQLSELECTL A.RECID FROM JSQLDEMO A, CATEGORY B WHERE B.DESCRIPTION = 'DATABASE' AND A.CATEGORY = B.ID
```

While you could achieve the same in jQL using a translate function to the CATEGORY file it is not a simple task to have
an index on the DESCRIPTION field as you would have to have triggers on both files and potentially perform
an expensive index updated if a DESCRIPTION changed.
...
054 JBASE_EDICT_END
```
Note that the dictionary id is still **PAID.DATE** which is not a valid SQL column but attribute 34
has a valid column name: **PAID_DATE**.
