...
One of the advantages of using **SQLSELECTL** instead of regular **{S}SELECT** is that in situations where you have a large table (file) and you want to get a result set based on a value in another table (file) that is (ideally) indexed.
...
While you could achieve the same in jQL using a translate function to the CATEGORY file it is not a simple task to have an index on the DESCRIPTION field as you would have to have triggers on both files and potentially perform an expensive index updated if a DESCRIPTION changed.
...
Note that the dictionary id is still **PAID.DATE** which is not a valid SQL column but attribute 34 has a valid column name: **PAID_DATE**.
