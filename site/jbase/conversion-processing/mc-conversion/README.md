...
| MCPU | Displays non-printable characters in the conventional way UNIX does, so 0x01 to 0x1A are @A to @Z , 0x00 is @^.<br>The other non-printable characters are \nnn (where nnn is the octal equivalent), and system delimiters are displayed as expected. |
